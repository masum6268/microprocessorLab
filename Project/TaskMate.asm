...
msg_header DB " _______           _     __  __         _         ",0DH, 0AH
           DB "|__   __|         | |   |  \/  |       | |        ",0DH, 0AH
           DB "   | |   _ _  ___ | | __| \  / |  __ _ | |_   ___ ",0DH, 0AH
           DB "   | | / _` |/ __|| |/ /| |\/| | / _` || __| / _ \",0DH, 0AH
           DB "   | || (_| |\__ \|   < | |  | || (_| || |_ |  __/",0DH, 0AH
           DB "   |_| \__,_||___/|_|\_\|_|  |_| \__,_| \__| \___|",0DH, 0AH
           DB "                                                     ",0DH, 0AH
           DB "                                                     ",0DH, 0AH
    ...
    msg_updating_header   DB "*************************************************", 0DH, 0AH 
                 ...
                 DB "", 0DH, 0AH
                 DB "$"
                 msg_search_header DB "*************************************************", 0DH, 0AH 
                 DB "*                                                 *", 0DH, 0AH
                 DB "*               SEARCH A TASK                     *", 0DH, 0AH
                 DB "*                                                 *", 0DH, 0AH
                 DB "*************************************************", 0DH, 0AH 
                 DB "", 0DH, 0AH
                 DB "$"
...
DELETE_TASK PROC
   ...
   SEARCH_TASK PROC
    CLEAR_SCREEN
    SHOW msg_search_header
    SHOW newline
    
    SHOW msg_search_prompt  ; Display the prompt message
    SHOW newline
    GET_STRING buffer
    
    ; Initialize a flag to track if a match is found
    MOV AL, 0  ; 0 means no match found
    
    ; Compare with the first task if at least one task exists
    CMP taskAdded, 0
    JLE NO_MATCH
    ...
    ; Compare with the second task if there are at least two tasks
    CMP taskAdded, 1
    JLE NO_MATCH
    ...
    ; Match found for the first task
    MATCH_FOUND1:
    SHOW newline
    SHOW msg_match_found
    SHOW newline
    SHOW_TASK "1", todo_text1, todo_date1
    SHOW newline
    SHOW msg_back
    ...
    ; Match found for the second task
    MATCH_FOUND2:
    SHOW newline
    SHOW msg_match_found
    SHOW newline
...
COMPARE_STRINGS PROC
    MOV CX, 100  ; Limit the comparison length
    MOV AL, 0    ; Default to no match
    
COMPARE_LOOP:
    LODSB         ; Load a byte from the source string
    SCASB         ; Compare it with a byte from the destination string
    JNE NOT_EQUAL ; Exit loop if they don't match
    
    CMP AL, '$'   ; Stop comparing if end of string is reached
    JE EQUAL
    LOOP COMPARE_LOOP

EQUAL:
    MOV AL, 1     ; Match found
    RET
